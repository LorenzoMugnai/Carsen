"""MCP server factory for Ariadne knowledge instances."""
...
def _server_class():
    try:
        ...
        raise MCPUnavailableError("The 'mcp' package is required to serve Ariadne over MCP") from exc
    return module.MCPServer


def create_mcp_server(config: AriadneConfig):
    """Create an MCP server exposing tools for one knowledge instance."""
    runtime = InstanceRuntime(config)
    server = _server_class()(f"ariadne-{config.knowledge.id}")
...
def run_mcp_server(config: AriadneConfig, transport: str | None = None) -> None:
