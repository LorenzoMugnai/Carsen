"""Configuration loading and validation for Carsen knowledge instances.

Carsen is configured per knowledge base.  This module validates one YAML file
...
class CarsenConfig(BaseModel):
    ...
    @model_validator(mode="after")
    def derive_instance_storage(self) -> CarsenConfig:
        ...
        if self.storage.data_directory is None:
            self.storage.data_directory = Path.home() / ".local" / "share" / "carsen" / self.knowledge.id
...
def _resolve_relative_paths(config: CarsenConfig, base_dir: Path) -> CarsenConfig:
...
def load_config(path: str | Path) -> CarsenConfig:
    ...
    try:
        return _resolve_relative_paths(CarsenConfig.model_validate(expand_env(raw)), config_path.parent)
    except ValidationError:
        raise


def default_config(name: str) -> CarsenConfig:
    """Build a default local configuration for a registry entry."""

    return CarsenConfig(knowledge=KnowledgeConfig(id=name, name=f"{name} Knowledge Base"))


def dump_config(config: CarsenConfig) -> str:
