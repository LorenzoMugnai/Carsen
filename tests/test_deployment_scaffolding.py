...
def test_dockerfile_and_ignore_keep_runtime_single_image() -> None:
    ...
    assert 'ENTRYPOINT ["carsen"]' in dockerfile
...
def test_compose_uses_one_image_for_multiple_instances() -> None:
    ...
    assert services["carsen-alpha"]["image"] == "carsen-mcp:latest"
    assert services["carsen-beta"]["image"] == "carsen-mcp:latest"
    assert services["carsen-alpha"]["ports"] == ["8765:8765"]
    assert services["carsen-beta"]["ports"] == ["8766:8766"]
    assert "docker-alpha.yaml" in " ".join(services["carsen-alpha"]["volumes"])
    assert "docker-beta.yaml" in " ".join(services["carsen-beta"]["volumes"])
...
def test_systemd_template_uses_instance_name_and_registry_convention() -> None:
    unit = read("deployment/systemd/carsen@.service")
    assert "Carsen Knowledge MCP instance %i" in unit
    assert "CARSEN_CONFIG_DIR=/etc/carsen" in unit
    assert "carsen serve %i --transport http" in unit
